...
# Hazard ratios from biopsy and resect samples
#  for 258 genes with significant results for survival
df1 <- read.table("HR_quadrant_plot_biopsy_resec335genes_wo15.txt",
                 header=TRUE, sep='\t')

# Hazard ratios from biopsy and resect samples
#  for ALL 335
df2 <- read.table("HR_quadrant_plot_biopsy_resec_ALL335genes.txt",
                 header=TRUE, sep='\t')
...
#df <- df1 %>% left_join(n335info)

df <- df2 %>% left_join(n335info)
...
scatplot1 <- ggplot(df, aes(x = pre.Tx, y =post.Tx, 
  ...
  scale_x_continuous(limits = c(0.25, 4.5),  n.breaks=7) +
  scale_y_continuous(limits = c(0.6, 1.5),n.breaks=7)+
...
p1 <- scatplot1 +
  ...
  annotate("text", x = 1, y = -Inf, 
           ...
           hjust=-1.75,vjust=-34, size=4, color = "blue") +

...
# Version: 2024-11-18
#
# Figure 2E, Figure S8F: Sankey plots of AIMS subtypes
# 
# !! IMPORTANT !!:
#
...
# Import the AIMS subtypes of pre-Tx biopsy and post-Tx resect sample

sank_df <- read.delim("BR_AIMSnew.txt", stringsAsFactors = T)

# Analysis / Plots ####

df <- sank_df %>%
  make_long(pre.Tx,post.Tx) |> mutate(node = forcats::fct_inorder(node))
...
ggsave( "./Fig2E_Sankey_plotBR_AIMS.pdf", pointsize = 12, bg = "white")

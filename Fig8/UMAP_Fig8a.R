...
# Version: 2024-11-14
#
# Figure 8A: UMAP color by AIMS-subtypes
# 
#
#
#
...
row.names(heatmap_br)<- heatmap_br$id 
heatmap_br$id <- NULL
...
AIMS.colors <- c('BasalL' = "red", 'Her2e' = "magenta", 'LumA' = 'darkblue', 'LumB' = "lightblue3", 'NormL' = "lightgreen")
...
AC_ggplot <- ggplot(
  ...
  geom_point(aes(color = pre.AIMS, shape = AIMS), size=3) +
  scale_color_manual(name = "pre.AIMS", values = AIMS.colors) +  
  new_scale_color() +  # Add a new color scale
  #stat_ellipse(aes(group = A.Cluster, color = A.Cluster), type = "norm", level = 0.95, size=1) +
  #scale_color_manual(name = "A.Cluster", values = AC.colors) +  
  scale_shape_manual(name = "AIMS", values = c(17,19)) +  
  labs(color = "Legend") +  
  theme_bw() + 
  theme(
...
AC_ggplot
ggsave("UMAP_AIMS_Fig8A.svg", plot = AC_ggplot, device = "svg", width = 10, height = 8)
